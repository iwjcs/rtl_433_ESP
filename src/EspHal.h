...
public:
  ...
  EspHal(int8_t sck, int8_t miso, int8_t mosi)
      : RadioLibHal(INPUT, OUTPUT, LOW, HIGH, RISING, FALLING),
        ...
        spiMISO(miso),
        spiMOSI(mosi) {
    printf(" called spiSCK %d, spiMISO: %d, MspiMOSIOSI: %d\n", sck, miso, mosi);
  }

  void init() override {
    // we only need to init the SPI here
    printf(" calling spiBegin\n");
    spiBegin();
    printf("spiBegin called!\n");
  }
  ...
  void pinMode(uint32_t pin, uint32_t mode) override {
    ...
    //printf(" calling pinMode pin %d, mode %d", pin, mode);
    //printf("\n");
